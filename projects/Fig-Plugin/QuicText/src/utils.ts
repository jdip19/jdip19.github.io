...
import { FontReference } from './types';

/**
 * Get all font ranges in a text node
 */
export async function loadAllFontsForNode(node: TextNode): Promise<boolean> {
 ...
 */
export function getAllUniqueFonts(textNodes: TextNode[]): FontName[] {
 ...
 * Load all fonts
 */
export async function loadAllFonts(fonts: FontName[]): Promise<boolean> {
 ...
 */
export function fontsEqual(a: FontReference, b: FontReference): boolean {
  if (a === b) {
    return true;
  }
 ...
 */
export function createFontKey(font: FontReference): string {
  if (font === figma.mixed) {
    return 'mixed';
  }
 ...
 */
export function getKeywordList(keywords: string): string[] {
  return keywords
    .split(',')
    .map((k: string) => k.trim())
 ...
 */
export function formatTime(format: string, date = new Date()): string {
